...
namespace icp {
	...
	class DoublePinyin {
		...
		public static Id query(int key1, int key2 = 0) {
			if (!scheme.contains(key1) || !scheme[key1].contains(key2)) return new Id("");
			return scheme[key1][key2];
		}

		// convert to full pinyin
		public static void convert(string double_pinyins, out Sequence sequence) {
			ArrayList<Id> ids = new ArrayList<Id>();
			for (int pos = 0; pos < double_pinyins.length; ++pos) {
				if (pos + 1 < double_pinyins.length) {
					Id id = query((int)double_pinyins[pos], (int)double_pinyins[pos+1]);
					if (!id.empty()) {
						pos++;
						ids.add(id);
					}
